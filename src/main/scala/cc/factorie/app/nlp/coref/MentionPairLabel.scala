...
import cc.factorie.app.nlp.coref.mention.{CorefFeatures, Mention}
...
class MentionPairFeatures(val model: PairwiseCorefModel, val mention1: Mention, val mention2: Mention, mentions: Seq[Mention], options: Coref1Options) extends BinaryFeatureVectorVariable[String] {
  ...
  def mentType(ment: Mention): String = if(ment.isPRO) "pro" else "non"
  ...
  def acronymMatch: Char = {
    if (mention1.length == 1 && mention2.length > 1) {
      if (mention2.acronym.contains(mention1.phrase.trim.toLowerCase)) 't' else 'f'
    } else if (mention1.length > 1 && mention2.length == 1) {
      if (mention1.acronym.contains(mention2.phrase.trim.toLowerCase)) 't' else 'f'
    } else
      'u'
  }
  ...
  def computeBasicFeatures() {
    ...
    if (mention1.sentence == mention2.sentence) addMergeableFeature("ss") // false values of this feature are not included in Roth's system
    CorefFeatures.matchingTokensRelations(mention1, mention2).foreach(r => addMergeableFeature("apr" + r))

    if (mention1.head.string.toLowerCase == mention2.head.string.toLowerCase)
      addMergeableFeature("bM")
    else addMergeableFeature("bMf")
    if (mention1.last.string.toLowerCase == mention2.last.string.toLowerCase)
      addMergeableFeature("eM")
    else addMergeableFeature("eMf")
    if (mention1.head.string == mention2.head.string)
      addMergeableFeature("bMc")
    else addMergeableFeature("bMcf")
    if (mention1.last.string == mention2.last.string)
    ...
    val sdist = bin(mention1.sentence.indexInSection - mention2.sentence.indexInSection, 1 to 10)
    if (binTokenSentenceDistances) for (sd <- 1 to sdist) addMergeableFeature("sd" + sd)
    else addMergeableFeature("sd" + sdist)
    val tdist = bin(mention1.start - mention2.start, Seq(1, 2, 3, 4, 5, 10, 20, 50, 100, 200))
